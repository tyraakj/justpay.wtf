...
export function HeroSection() {
  ...
  return (
    <div ref={container} className="relative min-h-[80vh] flex flex-col items-center justify-center pt-24 pb-12 px-6 overflow-hidden w-full">
      ...
      <div className="hidden md:block absolute inset-0 pointer-events-none overflow-hidden max-w-7xl mx-auto">
        <div className="floating-icon-1 absolute top-[20%] left-[10%] p-4 rounded-2xl bg-white/[0.03] border border-white/[0.08] backdrop-blur-[16px] shadow-2xl shadow-rose-500/10">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none opacity-30 md:opacity-50">
        <LiquidEther
          colors={['#10B981', '#F43F5E', '#059669']}
          mouseForce={20}
          cursorSize={100}
          isViscous
          viscous={30}
      ...
      {/* Main Content */}
      <div className="flex flex-col gap-6 md:gap-8 w-full max-w-4xl mx-auto text-center relative z-10 px-2">
        <h1 className="text-5xl md:text-6xl lg:text-7xl font-heading font-extrabold text-foreground tracking-tight leading-[1.1] sm:leading-none">
          <div className="overflow-hidden">
            <span className="hero-title-line block">The fastest way to</span>
          </div>
          <div className="overflow-hidden pb-2 md:pb-4">
            <span className="hero-title-line block text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-rose-400 to-emerald-400 drop-shadow-[0_0_30px_rgba(16,185,129,0.3)]">
              get paid in crypto.
            </span>
          </div>
        </h1>
        
        <p className="hero-subtitle text-lg md:text-2xl text-zinc-400 font-medium leading-relaxed max-w-2xl mx-auto mb-4 md:mb-8 px-4">
        ...
        <div className="hero-cta flex justify-center">
          <Link href="/dashboard" className="btn-primary text-base md:text-lg px-6 md:px-8 py-3 md:py-4 inline-flex items-center gap-2 w-full sm:w-auto justify-center">
